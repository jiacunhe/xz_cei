...
public class DataHelper {
	...
	public void savePreload(final Preload preload) {
		...
		contentvalues.put(Preload.LOAD_CURRENTBYTE,
				preload.getLoadCurrentByte());
		...
		contentvalues.put(
				Preload.LOAD_PARENTID,
				preload.getLoadParentId() == null ? "" : preload
						.getLoadParentId());
		contentvalues.put(Preload.PASS_KEY, preload.getPassKey() == null ? ""
				: preload.getPassKey());
		contentvalues.put(
				Preload.CLASS_LENGTH,
				preload.getClassLength() == null ? "0" : preload
						.getClassLength());
		contentvalues
				.put(Preload.CLASS_LEVEL, preload.getClassLevel() == null ? "1"
						: preload.getClassLevel());
	...
	public int updatePreload(Preload preload) {
		...
		int id = db.update(SqliteHelper.TB_PRELOAD_NAME, values,
				Preload.LOAD_PLAYID + "=?",
				new String[] { preload.getLoadPlayId() });
		return id;
	}
	...
	public int deletePreload(String playId) {
		int id = -1;
		id = db.delete(SqliteHelper.TB_PRELOAD_NAME,
				Preload.LOAD_PLAYID + "=?", new String[] { playId });
		return id;
	}
	...
	private void savePlayRecord(Courseware courseware) {
		...
		contentvalues.put(
				Courseware.CLASSLENGTH,
				courseware.getClassLength() == null ? "0" : courseware
						.getClassLength());
		contentvalues.put(Courseware.PROTIME,
				courseware.getProTime() == null ? "" : courseware.getProTime());
		contentvalues.put(
				Courseware.STUTIME,
				courseware.getStudyTime() == null ? "0" : courseware
						.getStudyTime());
		contentvalues.put(
				Courseware.TIMEPOINT,
				courseware.getTimePoint() == null ? "0" : courseware
						.getTimePoint());
		contentvalues.put(
				Courseware.ISCOMPLETED,
				courseware.getIscompleted() == null ? "0" : courseware
						.getIscompleted());
		contentvalues.put(Courseware.UPLOADTIME, courseware.getUploadTime());
		contentvalues.put(Courseware.CLASSLEVEL, courseware.getClassLevel());
		contentvalues.put(
				Courseware.FULLNAME,
				courseware.getFullName() == null ? "" : courseware
						.getFullName());
		contentvalues.put(Courseware.ORDERTIME, courseware.getOrderTime());
		db.insert(SqliteHelper.TB_STUDYRECORD_NAME, Courseware.ID,
				contentvalues);
		contentvalues.clear();
	}
	...
	public List<Courseware> getStudyRecord() {
		...
		while (!cursor.isAfterLast() && (cursor.getString(1) != null)) {
			...
			studyRecord.setClassLevel(cursor.getString(11));
			studyRecord.setFullName(cursor.getString(12));
			studyRecord.setOrderTime(cursor.getLong(13));
	...
	public void updatePlayRecord(Courseware courseware) {
		...
		db.update(SqliteHelper.TB_STUDYRECORD_NAME, contentvalues,
				Courseware.CLASSID + "=?",
	...
	public Boolean haveStudyRecord(String classId) {
		Boolean b = false;
		Cursor cursor = db.query(SqliteHelper.TB_STUDYRECORD_NAME, null,
				Courseware.CLASSID + "=?", new String[] { classId }, null,
				null, null);
		b = cursor.moveToFirst();
		cursor.close();
		return b;
