	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		/*if (coursewares.size() != 0) {
			...
			if (!imageUrl.toLowerCase().contains("null")
					...
					&& drawables.get(imageUrl) != null && drawables.get(imageUrl).get() != null) {
				holder.courseIcon.setImageDrawable(drawables.get(imageUrl).get());
			} else {
						...
						new AsyncImageLoader.ImageCallback() {
							...
							public void imageLoaded(Drawable drawable,
								...
								if (drawable != null && imageView != null) {
									...
									drawables.put(path, new SoftReference<Drawable>(drawable));
								}
							}
						});
			}
